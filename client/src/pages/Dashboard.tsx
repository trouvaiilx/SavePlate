...
import { getFoodItems, daysUntilExpiry, type FoodItem } from '@/lib/store';
import { Package, HeartHandshake, TrendingUp, AlertTriangle } from 'lucide-react';

export default function Dashboard() {
  const { user } = useAuth();
  const [items, setItems] = useState<FoodItem[]>([]);
  useEffect(() => { setItems(getFoodItems(user!.id)); }, []);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-6xl mx-auto px-5 py-8">
        ...
        <div className="mb-8 pb-6 border-b border-border">
          ...
          <h1 className="font-serif text-3xl text-foreground">{user?.full_name.split(' ')[0]}'s Food Dashboard</h1>
        </div>

        {/* Expiry alert bar */}
        {expiring.length > 0 && (
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-300 border-l-4 border-l-amber-500 px-4 py-3 mb-8">
            ...
            <Link to="/inventory" className="text-xs font-semibold text-amber-700 hover:underline underline-offset-2 shrink-0">
              View →
            </Link>
          </div>
        )}
        ...
        {/* Stats row */}
        <div className="grid grid-cols-2 sm:grid-cols-4 border border-border divide-x divide-y sm:divide-y-0 divide-border mb-10">
          {[
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <h2 className="font-serif text-lg mb-4">What do you want to do?</h2>
            <div className="divide-y divide-border border border-border">
              {[
                ...
                { to: '/analytics', icon: TrendingUp,     label: 'Food Analytics',         desc: 'Review your food-saving impact over time (Iteration 2)' },
                { to: '/meals',     icon: HeartHandshake, label: 'Weekly Meal Planner',    desc: 'Plan meals around expiring ingredients (Iteration 2)' },
              ].map(({ to, icon: Icon, label, desc }) => (
                <Link key={to} to={to} className="flex items-center gap-4 px-4 py-3.5 hover:bg-muted/50 transition-colors group">
          ...
          {/* Expiring list */}
          <div>
            <h2 className="font-serif text-lg mb-4">Needs attention</h2>
            {expiring.length === 0 ? (
              ...
              <div className="border border-border divide-y divide-border">
                ...
                <Link to="/inventory" className="block px-4 py-2 text-xs text-primary font-semibold hover:bg-muted/50 transition-colors">
